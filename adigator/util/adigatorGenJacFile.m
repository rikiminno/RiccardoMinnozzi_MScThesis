function output = adigatorGenJacFile(UserFunName,UserFunInputs, JacFileName, varargin)
...
AdiJacFileName = [char(JacFileName),'_ADiGatorJac']; % Name of derivative file
...
% NOTE: added by Riccardo Minnozzi, 06/2024
% change the Jacobian output name to be function output plus undersocre and
% vod name
ystr = FunctionInfo.Output.Names{1};
x = UserFunInputs{derflag};
vodname = x.deriv.vodname;
isAlreadyDer = checkIfOutputIsDer(ystr);
if isAlreadyDer
    JacOut = [ystr, vodname];
else
    JacOut = [ystr, '_', vodname];
end

% functionstr = ['function [Jac,Fun] = ',JacFileName,'(',InputStr1,')\n'];
functionstr = ['function ', JacOut, ' = ',JacFileName,'(',InputStr1,')\n'];
...
% NOTE: added by Riccardo Minnozzi, 06/2024
% check to see if the adigator-defined Jacobian has a defined derivative
% structure field
hasDer = findDerInAdiJac(AdiJacFileName, ystr, vodname);
if ~ hasDer
    % if the Jacobian has no defined derivative, add a line that
    % initializes the derivative to 0
    fprintf(fid, [ystr, '.d', vodname, ' = zeros(%1.0f,1);\n'], prod(x.func.size));
end
...
if dydxnnz == dydxnumel
    fprintf(fid,[JacOut, ' = reshape(',dy,',[%1.0f %1.0f]);\n'],dydxsize);
elseif dydxsize(1) == 1 && dydxsize(2) == 1
    fprintf(fid,[JacOut, ' = ',dy,';\n']);
elseif dydxsize(1) == 1
    fprintf(fid,[JacOut, ' = zeros(1,%1.0f); \n'],dydxsize(2));
    if hasDer
        fprintf(fid,[JacOut, '(',dy,'_location) = ',dy,';\n']);
    end
elseif dydxsize(2) == 1
    fprintf(fid,[JacOut, ' = zeros(%1.0f,1); \n'], dydxsize(1));
    if hasDer
        fprintf(fid,[JacOut, '(',dy,'_location) = ',dy,';\n']);
    end
else
    ...
    if dydxnumel >= 250 && dydxnnz/dydxnumel <= 3/4 && hasDer
        % Project Sparse
        fprintf(fid,[JacOut, ' = sparse(',rowstr,',',colstr,',',dy,',%1.0f,%1.0f);\n'],dydxsize);

        % NOTE: added by Riccardo Minnozzi, 06/2024
        fprintf(fid, [JacOut, ' = full(', JacOut, ');\n']);
    else
        % Project Full
        fprintf(fid,[JacOut, ' = zeros(%1.0f,%1.0f);\n'],dydxsize);
        if hasDer
            fprintf(fid,[JacOut, '((',colstr,'-1)*%1.0f+',rowstr,') = ',dy,';\n'],dydxsize(1));
        end
    end
end
...
% NOTE: all the hasDer if checks have been added by Riccardo Minnozzi to
% allow the computation of a second derivative even when the first
% derivative is zero, and to remove the function evaluation from the Jac
% generator since it also causes issues with recursive calls for higher
% order derivatives


function hasDerivative = findDerInAdiJac(adigatorJacFun, ystr, vodName)
% function to find if the derivative with respect to the vod is defined in
% the adigator-generated Jacobian file

% read the adigator generated Jaccobian
adigatorJacFun = strcat(string(adigatorJacFun), ".m");
adiFunContent = readlines(adigatorJacFun);

derDeclaration = strcat(ystr, ".d", vodName, " =");
hasDerivative = 0;
for i = 1:length(adiFunContent)
    % read each line

    if contains(adiFunContent(i), derDeclaration)
        hasDerivative = 1;
    end
end
end
...
function isAlreadyDer = checkIfOutputIsDer(ystr)
% function that checks whether the output of the differentiated function is
% already a derivative or not ()

isAlreadyDer = 0;
if contains(string(ystr), "_")
    isAlreadyDer = 1;
end
end
